...
<div class="flex justify-center items-center flex-col mt-10">
    ...
    <form method="POST" action="{{ route('tasks.update', $task) }}" class="w-full lg:w-[600px]">
        @csrf
        @method('PUT')
        <div class="space-y-12">
            ...
            <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                ...
                <div class="sm:col-span-6">
                    ...
                    <div class="mt-2">
                        <input id="title" required type="text" name="title" value="{{ $task->title }}" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
                    </div>
                </div>
                ...
                <div class="col-span-full">
                    ...
                    <div class="mt-2">
                        <textarea id="description" name="description" rows="3" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6">{{ $task->description }}</textarea>
                    </div>
