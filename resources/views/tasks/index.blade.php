        ...
        @foreach ($tasks as $t)
            <div class="bg-[#f1faee] px-6 py-4 rounded-xl flex flex-col gap-2">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex justify-between items-center">
                        <form action="{{ route('tasks.changeStatus', $t) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <select name="status" id="status" onchange="this.form.submit()">
                                <option class="text-black" value="in_progress" {{ $t->status === "in_progress" ? 'selected' : ''}}>In progress</option>
                                <option class="text-black" value="completed" {{ $t->status === "completed" ? 'selected' : ''}}>Completed</option>
                                <option class="text-black" value="pending" {{ $t->status === "pending" ? 'selected' : ''}}>Not started</option>
                            </select>
                        </form>
